...
class Video(models.Model):
    ...
    video_id = models.CharField(max_length=40, unique=True) # enforcing unique id numbers

    def save(self, *args, **kwargs):
        # extract the video id from a youtube url
        if not self.url.startswith('https://www.youtube.com/watch'):
            raise ValidationError(f'Not a YouTube video: {self.url}')
    ...
    def __str__(self):
        return f'ID: {self.pk}, Name: {self.name}, URL: {self.url}, VideoID: {self.video_id}, Notes: {self.notes[:200]}'